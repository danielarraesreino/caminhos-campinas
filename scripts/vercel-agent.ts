...
function processLogs(rawLogs: string) {
	...
	lines.forEach((line) => {
		...
		try {
			...
			if (
				...
				(entry.proxy && entry.proxy.duration > 500)
			) {
				console.log(
					`⚠️  [${entry.statusCode}] ${entry.request.method} ${entry.request.uri} (${entry.proxy?.duration}ms)`,
				);
			}
		} catch (e) {
	...
	if (errors === 0 && slowRequests === 0) {
		console.log(
			"✅ SISTEMA ESTÁVEL. Nenhuma anomalia crítica detectada nos logs recentes.",
		);
	} else {
		console.log(
			"⚠️  ATENÇÃO: Existem problemas potenciais. Verifique os detalhes acima.",
		);
	}
}
