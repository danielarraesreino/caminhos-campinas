...
async function validate() {
	...
	// 1. Load Data
	const dilemmasPath = path.join(
		process.cwd(),
		"src/data/dilemmas-campinas.json",
	);
	const servicesPath = path.join(
		process.cwd(),
		"src/data/services-campinas.json",
	);

	if (!fs.existsSync(dilemmasPath))
		throw new Error("dilemmas-campinas.json not found");
	if (!fs.existsSync(servicesPath))
		throw new Error("services-campinas.json not found");
	...
	dilemmas.forEach((d: any) => {
		...
		if (
			d.nextDilemmaId &&
			!dilemmaIds.has(d.nextDilemmaId) &&
			d.nextDilemmaId !== "CREDITS_SCREEN" &&
			d.nextDilemmaId !== "RESTART_GAME"
		) {
			console.error(
				`❌ ERROR: Dilemma '${d.id}' points to missing nextDilemmaId '${d.nextDilemmaId}'`,
			);
			errors++;
		}
		d.options.forEach((opt: any, idx: number) => {
			if (
				opt.nextDilemmaId &&
				!dilemmaIds.has(opt.nextDilemmaId) &&
				opt.nextDilemmaId !== "CREDITS_SCREEN" &&
				opt.nextDilemmaId !== "RESTART_GAME"
			) {
				console.error(
					`❌ ERROR: Dilemma '${d.id}' option ${idx} points to missing nextDilemmaId '${opt.nextDilemmaId}'`,
				);
				errors++;
			}
		});
		...
		if (
			d.trigger &&
			d.trigger.condition &&
			typeof d.trigger.condition === "string"
		) {
			try {
