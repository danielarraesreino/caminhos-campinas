...
test.describe('Exclusão Digital e Resiliência', () => {
    ...
    test.beforeEach(async ({ page }) => {
        ...
        if (await avatarHeader.isVisible({ timeout: 5000 })) {
            ...
            await page.locator('select[title="Selecione o gênero"]').selectOption('masculino');
            await page.locator('select[title="Selecione a faixa etária"]').selectOption('adulto');
            ...
            await page.getByRole('button', { name: 'preto', exact: false }).first().click();
    ...
    test('Cenário 1: Navegabilidade Offline (Service Worker)', async ({ page, context }) => {
        ...
        await expect(page.locator('header svg.text-red-500')).toBeVisible({ timeout: 10000 });
    ...
    test('Cenário 2: Bateria Baixa (Bloqueio de Chat)', async ({ page }) => {
        ...
        await expect(page.getByText('Você está digitalmente invisível')).toBeVisible();
