...
export function SyncButton() {
    ...
    useEffect(() => {
        const checkData = () => {
            ...
            // Check telemetry
            const telemetryStored = localStorage.getItem("caminhos_campinas_telemetry");
    ...
    const handleSync = async () => {
        ...
        try {
            ...
            const telemetryEvents = telemetryStored ? JSON.parse(telemetryStored) : [];
            const userStories = suggestionsStored ? JSON.parse(suggestionsStored) : [];
    ...
    return (
        <div className="flex items-center gap-2">
            <Button
                ...
                className={`relative gap-2 transition-all ${hasData ? "border-blue-500/50 text-blue-400 hover:bg-blue-950/30" : "opacity-70"
            ...
            >
                ...
                <span className="hidden sm:inline">
                    {status === "success" ? "Enviado!" : status === "error" ? "Erro" : "Sincronizar"}
                </span>
            ...
            {msg && (
                <span className={`text-xs font-mono animate-in fade-in slide-in-from-left-2 ${status === "error" ? "text-red-400" :
