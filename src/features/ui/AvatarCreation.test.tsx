...
describe("AvatarCreation", () => {
  ...
  const navigateToStep2 = async (user: ReturnType<typeof userEvent.setup>) => {
    ...
    // Step 1: Name
    const nameInput = screen.getByPlaceholderText("Ex: Zé do Pátio, Maria da Praça...");
    ...
    // Now on Step 2
    expect(screen.getByText("Selecione uma Imagem de Identidade")).toBeInTheDocument();
  };
  ...
  it("handles AI image generation success", async () => {
    ...
    await waitFor(() => {
      expect(mockShowToast).toHaveBeenCalledWith("Retrato gerado com sucesso!", "success");
    });
