...
import { Lock, Zap, MousePointer2, HelpCircle, XCircle } from "lucide-react";
...
const TOOLTIP_DATA: Record<string, TooltipContent> = {
    "Topo": {
        ...
        impact: "Sem a base (moradia/saúde), o cérebro permanece em modo de sobrevivência, tornando o aprendizado técnico ou acadêmico biologicamente inviável.",
        reality: "Apenas 15% da população de rua em Campinas consegue acessar cursos de capacitação com sucesso."
    },
    "Meio_A": {
        title: "Dignidade e Estima Social",
        impact: "O estigma da rua destrói o pertencimento. Sem documentos ou um local de higiene, o indivíduo perde a percepção de si como cidadão.",
        reality: "71% citam o rompimento de vínculos familiares como a porta de entrada para a exclusão total."
    },
    "Meio_B": {
        title: "Segurança e Moradia",
        impact: "A falta de um teto (ODS 11.1) gera um estado de alerta perpétuo. O sono fragmentado na calçada impede a recuperação psíquica.",
        reality: "Campinas possui déficit de vagas em abrigos públicos em relação à demanda sazonal de inverno."
    },
    "Base": {
        title: "Necessidades Fisiológicas",
        impact: "Fome e dor física. Quando esta base falha, a ética social é atropelada pela necessidade biológica de se manter vivo.",
        reality: "O 'Tráfico Formiga' é o principal motor de encarceramento da pobreza quando a base de Maslow desmorona."
    }
};
                ...
                {isBaseBroken && (
                    <div className="mt-8 md:mt-0 md:absolute top-1/2 -right-16 transform -translate-y-1/2 flex flex-col items-center group cursor-help">
                        <div className="bg-red-600 text-white font-black p-5 rounded-xl shadow-[0_0_30px_rgba(220,38,38,0.6)] flex items-center gap-3 animate-pulse border-2 border-red-300">
                            ...
                            <span className="text-base uppercase tracking-tighter">Colapso Sistêmico</span>
                        </div>
                        ...
                        <div className="bg-black/90 p-3 border border-red-900 rounded-lg max-w-[180px] text-center">
                            <span className="text-[10px] text-red-500 font-bold uppercase block mb-1">Impacto Criminal</span>
                            <p className="text-[10px] text-slate-300 leading-tight">
                                A falha biológica na base força o recrutamento pelo tráfico para subsistência imediata.
                            </p>
                        </div>
                    </div>
                )}
                ...
                <div className="absolute inset-0 bg-black/95 z-50 flex flex-col p-8 rounded-2xl border-2 border-blue-900/50 animate-in fade-in zoom-in duration-200">
                    ...
                    <div className="space-y-6">
                        <div className="space-y-1">
                            <span className="text-blue-500 font-bold tracking-widest text-xs uppercase">Detalhes do Nível</span>
                            <h3 className="text-3xl font-black text-white">{TOOLTIP_DATA[activeTooltip].title}</h3>
                        </div>

                        <div className="space-y-4">
                            <div className="bg-blue-950/20 border-l-4 border-blue-600 p-4">
                                <span className="text-xs font-bold text-blue-400 uppercase block mb-2">Impacto na Sobrevivência</span>
                            ...
                            <div className="bg-slate-900/40 p-4 rounded-lg">
                                <span className="text-xs font-bold text-slate-500 uppercase block mb-2">Realidade Campinas</span>
...
function PyramidTier({ active, label, width, locked, alert, shaking, level, tooltipId, onInfoClick }: any) {
    return (
        <div className={`${width} min-h-[70px] border-2 transition-all duration-300 flex items-center justify-between px-6 py-4 relative group rounded-xl
        ...
        `}>
            ...
            <div className="flex flex-col gap-1 items-start">
                <span className="text-[10px] uppercase tracking-widest font-black text-slate-600 group-hover:text-blue-500 transition-colors">{level}</span>
            ...
            {!locked && (
                ...
                >
                    <span className="text-[10px] font-bold opacity-0 group-hover/btn:opacity-100 transition-opacity">Detalhes</span>
                    <HelpCircle size={20} />
                </button>
            )}
        </div>
