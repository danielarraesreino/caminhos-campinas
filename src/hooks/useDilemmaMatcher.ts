...
export function useDilemmaMatcher() {
    ...
    const findMatch = useCallback((userInput: string, userCoords: [number, number] | null) => {
        ...
        const locationObj = userCoords ? { lat: userCoords[0], lng: userCoords[1] } : null;
        ...
        if (input.includes("fome") && locationObj) {
            const bomPrato = services.find(s => s.name.toLowerCase().includes("bom prato"));
            if (bomPrato) {
                const dist = calculateDist(locationObj.lat, locationObj.lng, bomPrato.coords[0], bomPrato.coords[1]);
                if (dist <= 500) {
                    return {
                        id: "dynamic_fome_bp",
                        ...
                        options: [
                            {
                                ...
                                action: (state: any) => ({ ...state, hunger: Math.min(100, state.hunger + 50), money: state.money - 1 })
                            }
                        ]
        ...
        if ((input.includes("dor") || input.includes("machucado")) && locationObj) {
            const consultorio = services.find(s => s.type === "health" || s.name.includes("Consultório"));
            if (consultorio) {
                const dist = calculateDist(locationObj.lat, locationObj.lng, consultorio.coords[0], consultorio.coords[1]);
                    ...
                    options: [
                        {
                            ...
                            action: (state: any) => ({ ...state, health: Math.min(100, state.health + 20) })
                        }
                    ]
        ...
        const match = DilemmaMatcher.findBestDilemma(userInput, locationObj, dilemmas, serviceLocations);
        setLastMatch(match);
        return match;
