...
export function useODSTracker() {
    const trackAction = useCallback(async (actionName: string, odsTarget: string, metadata: Record<string, unknown> = {}) => {
    ...
    const trackServiceUse = useCallback(async (serviceName: string, serviceType: ServiceType) => {
        const ods = SERVICE_ODS_MAP[serviceType] || "ODS 1 - Erradicação da Pobreza";

        await trackAction(
            `USO_SERVICO_${serviceType.toUpperCase()}`,
            ods,
            {
    ...
    const trackDilemmaDecision = useCallback(async (dilemmaId: string, choiceLabel: string, odsTag?: string) => {
        await telemetryService.track(
