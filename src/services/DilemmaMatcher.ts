...
import { Dilemma } from "@/features/game-loop/dilemmas";
...
export const DilemmaMatcher = {
    findBestDilemma(
        ...
        for (const dilemma of relevantDilemmas) {
            ...
            else if (dilemma.trigger?.type === "LOCATION" && dilemma.trigger.locationId && userLocation) {
            ...
            else if (!dilemma.location_trigger && dilemma.trigger?.type !== "LOCATION") {
                if (!bestMatch || (!bestMatch.location_trigger && bestMatch.trigger?.type !== "LOCATION")) {
                    bestMatch = dilemma;
                }
            }
        }
