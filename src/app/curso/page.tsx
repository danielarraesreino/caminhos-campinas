...
import { ArrowLeft, BookOpen, Download, HardHat, Phone, Send, User, Users } from "lucide-react";
import Link from "next/link";
import { useState } from "react";

export default function CoursePage() {
    const [contactMethod, setContactMethod] = useState<"whatsapp" | "email" | "proxy">("whatsapp");
            ...
            <div className="max-w-4xl mx-auto space-y-8 p-6 pt-24">
                ...
                <header className="flex items-center gap-4 border-b border-slate-800 pb-6">
                    ...
                    <div>
                        ...
                        <p className="text-slate-400">Capacitação Técnica e Redução de Danos</p>
                    </div>
                </header>
                ...
                <div className="grid md:grid-cols-2 gap-12">
                    ...
                    <div className="space-y-8">
                        <section className="space-y-4">
                            ...
                            <p className="text-slate-300 leading-relaxed">
                                Formação gratuita para pessoas com trajetória de rua. Aprenda a usar
                                dados, leis e tecnologia para transformar a sua realidade e a da sua
                                comunidade.
                            </p>
                        </section>
                    ...
                    <div className="bg-slate-900/50 border border-slate-800 p-8 rounded-2xl">
                        <div className="mb-6">
                            <h3 className="text-xl font-bold text-white mb-1">Inscrição Rápida</h3>
                            <p className="text-sm text-slate-400">Garanta sua vaga na lista de espera.</p>
                        </div>
                        ...
                        {submitted ? (
                            <div className="bg-emerald-500/10 border border-emerald-500/20 p-6 rounded-xl text-center space-y-2 animate-in fade-in zoom-in">
                                ...
                                <p className="text-sm text-emerald-200">Entraremos em contato em breve.</p>
                                <Button onClick={() => setSubmitted(false)} variant="ghost" className="text-xs text-slate-400 mt-4 h-auto p-0 hover:text-white hover:bg-transparent">
                                    Nova inscrição
                                </Button>
                            </div>
                        ) : (
                            <form onSubmit={handleSubmit} className="space-y-5">
                                <div className="space-y-2">
                                    ...
                                    <Input id="name" placeholder="Como você quer ser chamado?" required className="bg-slate-950 border-slate-800 focus:border-blue-500 text-white" />
                                </div>
                                ...
                                <div className="space-y-4 p-4 bg-slate-950 rounded-xl border border-slate-800/50">
                                    {contactMethod === "whatsapp" && (
                                        <div className="space-y-2 animate-in slide-in-from-left-2">
                                            ...
                                            <div className="relative">
                                                ...
                                                <Input id="phone" type="tel" placeholder="(19) 99999-9999" required className="pl-9 bg-slate-900 border-slate-800 text-white" />
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    {contactMethod === "email" && (
                                        <div className="space-y-2 animate-in slide-in-from-left-2">
                                            ...
                                            <Input id="email" type="email" placeholder="exemplo@email.com" required className="bg-slate-900 border-slate-800 text-white" />
                                        </div>
                                    )}
                                    ...
                                    {contactMethod === "proxy" && (
                                        <div className="space-y-4 animate-in slide-in-from-left-2">
                                            <div className="space-y-2">
                                                <Label htmlFor="proxyName" className="text-purple-300">Nome da Pessoa de Referência</Label>
                                                <div className="relative">
                                                    <User className="absolute left-3 top-2.5 h-4 w-4 text-purple-500" />
                                                    <Input id="proxyName" placeholder="Ex: Maria (Assistente Social)" required className="pl-9 bg-slate-900 border-slate-800 text-white" />
                                                </div>
                                            </div>
                                            <div className="space-y-2">
                                                <Label htmlFor="proxyService" className="text-purple-300">Local de Referência (Serviço)</Label>
                                                <div className="relative">
                                                    <HardHat className="absolute left-3 top-2.5 h-4 w-4 text-purple-500" />
                                                    <Select required>
                                                        <SelectTrigger id="proxyService" className="pl-9 bg-slate-900 border-slate-800 text-white">
                                                            <SelectValue placeholder="Selecione o local..." />
                                                        </SelectTrigger>
                                                        <SelectContent className="bg-slate-900 border-slate-800 text-white">
                                                            <SelectItem value="caps_ad">CAPS AD</SelectItem>
                                                            <SelectItem value="centro_pop">Centro Pop</SelectItem>
                                                            <SelectItem value="consultorio">Consultório na Rua</SelectItem>
                                                            <SelectItem value="albergue">Albergue / Samim</SelectItem>
                                            ...
                                            <p className="text-xs text-slate-500 italic">
                                                * Entraremos em contato com esta pessoa para te avisar sobre o curso.
                                            </p>
                                        </div>
                                    )}
                                </div>
