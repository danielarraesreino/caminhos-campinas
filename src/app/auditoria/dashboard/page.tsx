...
export default function AdminDashboard() {
	...
	const [activeTab, setActiveTab] = useState<"partners" | "stories">(
		"partners",
	);
				...
				<header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
					<div className="space-y-2">
						...
						<p className="text-slate-400">
							Visualização de dados brutos e submissões da comunidade.
						</p>
					</div>
					...
					<Button
						onClick={fetchData}
						variant="outline"
						className="border-slate-800 bg-slate-900/50 hover:bg-slate-800 gap-2"
					>
						...
						{activeTab === "partners" ? (
							partners.length === 0 ? (
								...
								partners.map((p) => (
									<div
										key={p.id}
										className="bg-slate-900/50 border border-slate-800 p-5 rounded-2xl hover:border-blue-500/50 transition-all flex flex-col justify-between group"
									>
										<div>
							...
							stories.map((s) => (
								<div
									key={s.id}
									className="bg-slate-900/50 border border-slate-800 p-5 rounded-2xl hover:border-purple-500/50 transition-all group"
								>
									<div className="flex justify-between items-start mb-4">
