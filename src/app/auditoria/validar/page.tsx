...
const MOCK_DILEMMA = {
	...
	description:
		"São 17h30. Você está na região do Mercado Municipal. Um comerciante oferece R$ 20,00 para você descarregar um caminhão. O SAMIM fecha às 19h00 (tolerância zero). O trajeto a pé leva 40 minutos. Se você aceitar o trabalho, corre risco de perder o pernoite.",
	consequence:
		"Perda da vaga no abrigo; Exposição ao frio/risco noturno; Ganho financeiro imediato mas exclusão do sistema de acolhimento.",
};
				...
				<div className="max-w-4xl mx-auto flex justify-between items-center">
					<div>
						...
						<p className="text-xs text-slate-400">
							Ambiente de Validação Profissional (CRP/CRESS)
						</p>
					</div>
					...
					<form onSubmit={handleSubmit} className="space-y-6">
						...
						<div className="space-y-3">
							<p className="text-sm font-medium text-slate-700">
								Este cenário reflete a realidade das ruas de Campinas?
							</p>
							<div className="flex gap-4">
								<button
									type="button"
									onClick={() => setFeedback("pass")}
									className={`flex-1 py-3 rounded-xl border flex items-center justify-center gap-2 font-bold transition-all ${feedback === "pass" ? "bg-green-600 text-white border-green-600 ring-2 ring-green-200" : "border-slate-200 text-slate-500 hover:bg-slate-50"}`}
								>
								...
								<button
									...
									className={`flex-1 py-3 rounded-xl border flex items-center justify-center gap-2 font-bold transition-all ${feedback === "fail" ? "bg-red-600 text-white border-red-600 ring-2 ring-red-200" : "border-slate-200 text-slate-500 hover:bg-slate-50"}`}
								>
						...
						<div className="pt-4">
							<button
								type="button"
								className="w-full mb-3 py-3 bg-slate-100 text-slate-500 rounded-xl font-medium hover:bg-slate-200 transition-colors"
							>
								Pular Cenário
							</button>
							<button
								...
								className="w-full py-4 bg-blue-600 text-white rounded-xl font-bold shadow-lg shadow-blue-500/20 hover:bg-blue-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
							>
								Registrar Validação
							</button>
						</div>
